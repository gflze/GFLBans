...
from gflbans.internal.database.audit_log import EVENT_NEW_SERVER, DAuditLog, EVENT_EDIT_SERVER
...
from gflbans.internal.models.api import Server, PlayerObjNoIp, ServerInternal, PlayerObj, FileInfo
from gflbans.internal.models.protocol import AddServerReply, AddServer, EditServer, RegenerateServerTokenReply
...
def dserver_to_server(dsrv: DServer) -> Server:
    ...
    if dsrv.server_info is not None and (datetime.now(tz=UTC).replace(tzinfo=None)
                                         - dsrv.server_info.last_updated).total_seconds() <= 900:
        mf.online = True
...
def _duserip_to_ply(ply: DUserIP) -> PlayerObj:
    if ply.gs_avatar:
        return PlayerObj(gs_service=ply.gs_service, gs_id=ply.gs_id, gs_name=ply.gs_name,
                         gs_avatar=FileInfo(name=ply.gs_avatar.file_name,
                                            file_id=ply.gs_avatar.gridfs_file))
    else:
        return PlayerObj(gs_service=ply.gs_service, gs_id=ply.gs_id, gs_name=ply.gs_name)
...
async def get_players(request: Request, server_id: str):
    ...
    if sv.server_info is None or (datetime.now(tz=UTC).replace(tzinfo=None) - sv.server_info.last_updated).total_seconds() > 900:
...
@server_router.get('/{server_id}/internal', response_model_exclude_none=True, response_model_exclude_unset=True,
                   response_model=ServerInternal)
async def get_server_internal(request: Request, server_id: str,
    ...
    return ServerInternal(id=str(sv.id), ip=sv.ip, game_port=sv.game_port, enabled=sv.enabled,
                          friendly_name=sv.friendly_name, allow_unknown=sv.allow_unknown,
                          discord_staff_tag=sv.discord_staff_tag, discord_webhook_set=sv.discord_webhook is not None, infract_webhook_set=sv.infract_webhook is not None)


@server_router.post('/', response_model_exclude_unset=True, response_model_exclude_none=True,
                    response_model=AddServerReply, dependencies=[Depends(csrf_protect)])
async def create_server(request: Request, n: AddServer,
                        auth: Tuple[int, Optional[ObjectId], int] = Depends(check_access)):
    if auth[0] == NOT_AUTHED_USER: raise HTTPException(detail='This route requires authentication', status_code=401)
    ...
    dsv = DServer(ip=str(n.ip), game_port=n.game_port, friendly_name=n.friendly_name, allow_unknown=n.allow_unknown,
                  discord_webhook=n.discord_webhook, infract_webhook=n.infract_webhook, discord_staff_tag=n.discord_staff_tag, server_key=key_hash,
    ...
    await DAuditLog(time=datetime.now(tz=UTC), event_type=EVENT_NEW_SERVER, initiator=i,
                    message=ev_string, key_pair=(auth[0], auth[1])).commit(request.app.state.db[MONGO_DB])

    return AddServerReply(server_secret_key=key,
                          server=ServerInternal(id=str(dsv.id), ip=dsv.ip, game_port=dsv.game_port, enabled=dsv.enabled,
                                                friendly_name=dsv.friendly_name, allow_unknown=dsv.allow_unknown,
...
@server_router.patch('/{server_id}', response_model_exclude_none=True, response_model_exclude_unset=True,
                     response_model=ServerInternal, dependencies=[Depends(csrf_protect)])
async def edit_server(request: Request, e: EditServer, server_id: str,
                      auth: Tuple[int, Optional[ObjectId], int] = Depends(check_access)):
    if auth[0] == NOT_AUTHED_USER: raise HTTPException(detail='This route requires authentication', status_code=401)
    ...
    await DAuditLog(time=datetime.now(tz=UTC), event_type=EVENT_EDIT_SERVER, initiator=i,
                    message=ev_string, key_pair=(auth[0], auth[1]),
                    long_message=modifications).commit(request.app.state.db[MONGO_DB])

    return ServerInternal(id=str(srv.id), ip=srv.ip, game_port=srv.game_port, enabled=srv.enabled,
                          friendly_name=srv.friendly_name, allow_unknown=srv.allow_unknown,
...
@server_router.get('/{server_id}/token', response_model_exclude_none=True, response_model_exclude_unset=True,
                   response_model=RegenerateServerTokenReply, dependencies=[Depends(csrf_protect)])
