...
from gflbans.internal.map_images import find_map_image, conv_map_image
...
async def get_map_image(request: Request, mod_name: str, map_name: str):
    ...
    return StreamingResponse(gridfs_read(), media_type='image/webp', headers={'Content-Length': str(fos.length),
...
@map_image_router.post('/{mod_name}/{map_name}', dependencies=[Depends(max_map_img_size)],)
async def write_map_image(request: Request, mod_name: str, map_name: str, contents: UploadFile = File(...),
    ...
    file_id = await gridfs_client.upload_from_stream(f'{mod_name}.{map_name}.webp', converted,
                                                     metadata={
                                                         'map_image': True,
                                                         'mod_name': mod_name,
                                                         'map_name': map_name,
