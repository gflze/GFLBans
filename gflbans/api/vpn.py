...
from gflbans.internal.models.api import AdminMinimal, Initiator, PositiveIntIncl0, VPNInfo
from gflbans.internal.models.protocol import AddVPN, FetchBlocklistReply, FetchWhitelistReply, RemoveVPN
...
@vpn_router.get(
    '/whitelist',
    response_model=FetchWhitelistReply,
    response_model_exclude_none=True,
    response_model_exclude_unset=True,
    dependencies=[Depends(ensure_management_privs)],
)
async def fetch_whitelist(
    request: Request, skip: PositiveIntIncl0, limit: conint(gt=0, le=50), sort=('ips_user', ASCENDING)
):
...
async def search_whitelist(request: Request, query: str, limit: conint(gt=0, le=50), skip: PositiveIntIncl0):
    qc = await do_whitelist_search(query)

    ais = []

    async for dadm in DAdmin.from_query(
        request.app.state.db[MONGO_DB], qc, limit=limit, skip=skip, sort=('ips_user', ASCENDING)
    ):
        gridfs_file = None
...
async def delete_whitelist(request: Request, admin: Initiator):
    try:
        adm = await load_admin_from_initiator(request.app, admin)
    except NoSuchAdminError:
        raise HTTPException(status_code=404, detail='The admin could not be found')
