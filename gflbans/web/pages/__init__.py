...
async def sctx(request: Request) -> dict:
    ...
    return {'request': request, 'opposite_theme': opposite_theme, 'user': user, 'p_nodes': name2perms, 'GB_VERSION': GB_VERSION,
            'GB_BEGIN_RENDER': datetime.now(tz=UTC).timestamp(), 'BRANDING': BRANDING, 'DISABLE_GUIDELINES': DISABLE_GUIDELINES}
